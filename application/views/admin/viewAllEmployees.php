      ...
      <option value="all" <?php echo ($limit=='all')?' selected="selected"':''; ?>>All</option>
        ...
        <table class="table table-striped table-bordered table-hover" id="msgTable">
          ...
          <tbody>
            ...
            <?php if(($elist->user_type == 'SA' || $elist->user_type == 'A' || $elist->u_active == 'N')  ) { continue; }?>
            
            <tr class="odd gradeX" >
             
              
              <td >
			  <?php echo anchor('admin/viewEmployeeRead/'.$elist->empid,$elist->contact_name);?>
			  
			  </td>
              <td><?php echo $elist->sname.'/'.str_pad($elist->employeeCode, 2, '0', STR_PAD_LEFT); ?></td>
              <td class="hidden-480"><?php echo $elist->contact_email;?></td>
              <td class="hidden-480" ><?php echo $elist->designation;?></td>
              <td class="hidden-480" ><?php echo date('d-M-Y', $elist->doj);?></td>
              ...
              <td class="hidden-480" ><?php echo anchor('admin/addEmployees/'.$elist->empid, 'Edit');?> 
              <!--/ <?php echo anchor('admin/delEmployees/'.$elist->empid, 'Delete',array('class'=>'delete', 'onclick'=>"return confirmDialog();"));?>--></td>
            </tr>
...
<script>
		jQuery(document).ready(function() {			
			...
			var site = '<?php echo site_url('admin/viewEmployees'); ?>';
			$('#row').change(function(){
