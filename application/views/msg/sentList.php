...
<h3 class="page-title"> Sent Box<small>View Sent Box</small> </h3>
  ...
  <div class="span12"> 
    ...
    <div class="portlet box blue">
      <div class="portlet-title">
        <h4><i class="icon-globe"></i>Sent Box</h4>
     ...
     <thead>
      <tr>
      <th>
      <input type="checkbox" id="checkTop" class="group-checkable" style="opacity: 0;">
      </th>
      <th>Date Time</th>
      <th>Sent To</th>
      <th>Subject</th>
      ...
      <th>View</th>
       ...
       <tbody>
      <?php foreach($sentList->result() as $sdata){ ?>
     
      <?php 
	  if($this->session->userdata('global_comp') !=''){
	  $list = Messagemodel::companyEmployeeList($this->session->userdata('global_comp'));
	  if(count($list) ==0)
	   {
		 break;   
	   }
      ...
      <input type="hidden" id="msg_id" value="<?php echo $sdata->sentId; ?>" />
      <input type="checkbox"  name="check[]" class="group" id="checkbox" />
      
      </td>
      <td onclick="location.href='<?php echo site_url("message/ViewSent/".$sdata->sentId) ?>'">
	  ...
	  {
	     echo Messagemodel::FindName($to[$i]);
		 if($i != (count($to)-1) )
		 {
		  echo ', ';	 
		 }	  
	...
	echo '<img src="'.base_url().'images/icons/icon_attachment.gif'.'" alt="attach" />';	 
      ...
      <td>
      <a href="<?php echo site_url('message/ViewSent/'.$sdata->sentId) ?>">View</a>
      </td>
...
<script>
		jQuery(document).ready(function() {			
			...
			$('#rowSelect').change(function(){
				$row =$(this).val();
			location.href='<?php echo site_url("message/sent") ?>'+'/'+$row+'/'+$('#day').val()+'/'+$('#month').val()+'/'+$('#year').val();
		
				});
				
				$('#submit1').click(function(){
					location.href='<?php echo site_url("message/sent") ?>'+'/'+$('#rowSelect').val()+'/'+$('#day').val()+'/'+$('#month').val()+'/'+$('#year').val();
...
$.uniform.update();
	...
	if($ids !='')
	{
		...
		$.ajax({
				 type: "POST",
				 url: '<?php echo site_url('message/sent_delete');?>', 
				 data: {id: $ids},
				 dataType: "text",  
				 cache:false,
				 success: 
