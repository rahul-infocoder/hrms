...
<h3 class="page-title"> Leaves <small>View All Leaves</small> </h3>
  ...
  <div class="span12"> 
    ...
    <div class="portlet box light-grey">
      <div class="portlet-title">
        <h4><i class="icon-globe"></i>Leaves List</h4>
        ...
        <table class="table table-striped table-bordered table-hover" id="msgTable">
          <thead>
            <tr>
              ...
              <th class="hidden-480">Full Days</th>
              <th class="hidden-480">Half Days</th>
              <th class="hidden-480">Reason</th>
              <th class="hidden-480">Status</th>
            </tr>
          </thead>
          <tbody>
            <?php foreach($leaveData->result() as $clist){ ?>
            <tr class="odd gradeX">
              <td><input type="checkbox" class="checkboxes" value="1" /></td>
              <td><?php echo ($clist->sendDate ==0)?'Not Set':date('d-M-Y',$clist->sendDate); ?></td>
              <td ><?php echo date('d/m/Y', $clist->leavefrom);?></td>
              <td class="hidden-480"><?php echo date('d/m/Y', $clist->leaveto);?></td>
              <td class="hidden-480"><?php echo $clist->fullday;?></td>
              <td class="hidden-480"><?php echo $clist->halfday;?></td>
              <td class="hidden-480"><?php echo $clist->reason;?></td>
              <td class="hidden-480">
			  
			  <?php 
              if($clist->approved == 'N')
			  {
				  echo '<label class="btn mini blue">Waiting</label>';
				 }
				 else if($clist->approved == 'Y')
				 {
					 echo '<label class="btn mini green">APPROVED</label>';
				 }
				 else if($clist->approved == 'D')
				 {
					 echo '<label class="btn mini red">DISAPPROVED</label>';
				 }
...
<script>
		jQuery(document).ready(function() {			
			...
			App.init();
			var site = '<?php echo site_url('employees/viewAllLeaves'); ?>';
